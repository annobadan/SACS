...
for (i in seq_along(years)) {
  ...
  ###'######################################################################
  ###' 
  ###' Filter only District for now
  ###' => We want datasets only for Estimated Teacher Hires
  ###' 
  ###' 
  ...
  ### Differed process by years
  if (year_num %in% sprintf("%02d",seq(8, 9))){
    
    df <- df %>%
      mutate(Section = recode_factor(Section, 
                                     "C" = "Estimated Teacher Hires", 
                                     "D" = "High School Graduation Requirements", 
                                     .default = NA_character_))

  } else if (year_num %in% sprintf("%02d",seq(10, 17))) {
  ...
  save(df, file = paste0("CBEDS_District_Data_", year_num, ".rda"))
  write.dta(df, file = paste0("CBEDS_District_Data_", year_num, ".dta"))
