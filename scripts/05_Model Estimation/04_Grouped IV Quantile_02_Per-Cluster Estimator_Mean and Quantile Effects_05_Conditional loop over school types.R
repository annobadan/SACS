...
###' 
###' 05_Conditional Loop over school types
###' 
...
###' 20181202 JoonHo Lee - Add conditional loop over school types
###' 
###' 
...
### School Types
tabdf(df, SOC)
SOC_vec <- as.character(unique(df$SOC))
SOC_vec <- SOC_vec[SOC_vec != "K-12 Schools"]
...
for (i in seq_along(y_names_vec)){
  ...
  ### Generate the releveled "SOC" factor variables
  df <- df %>%
    mutate(SOC = relevel(SOC, SOC_vec[tp]))  
  ...
  model_mean <- df_nested %>%
    mutate(model = map(.x = df_nested$data, 
                       .f = ~safe_lm(formula = yvar ~ factor(SOC), 
                                     data = .x)), 
  ...
  df_result_mean_coef <- model_mean %>%
    mutate(coef = map(model_result, ~ tidy(.x))) %>%
    unnest(coef) %>%
    rename(statistic_est = statistic, 
           p.value_est = p.value)
  
  tabdf(df_result_mean_coef, idx_error)
  
  
  df_result_mean_fit <- model_mean %>%
    mutate(fit = map(model_result, ~ glance(.x))) %>%
    unnest(fit)

  tabdf(df_result_mean_fit, idx_error)
  
  
  df_result_mean <- df_result_mean_coef %>%
    dplyr::select(-idx_error) %>%
    left_join(df_result_mean_fit, by = c("CountyCode", "DistrictCode", 
                                         "AcademicYear"))
  ...
  for (j in seq_along(quantile_vec)){
    ...
    df_result_quantile_coef <- model_quantile %>%
      mutate(coef = map(model_result, ~ tidy(.x))) %>%
      unnest(coef) %>%
      rename(statistic_est = statistic, 
             p.value_est = p.value)
    
    tabdf(df_result_quantile_coef, idx_error)
    
    
    df_result_quantile_fit <- model_quantile %>%
      mutate(fit = map(model_result, ~ glance(.x))) %>%
      unnest(fit)
    
    tabdf(df_result_quantile_fit, idx_error)
    
    
    df_result_quantile <- df_result_quantile_coef %>%
      dplyr::select(-idx_error) %>%
      left_join(df_result_quantile_fit, by = c("CountyCode", "DistrictCode", 
                                           "AcademicYear"))
