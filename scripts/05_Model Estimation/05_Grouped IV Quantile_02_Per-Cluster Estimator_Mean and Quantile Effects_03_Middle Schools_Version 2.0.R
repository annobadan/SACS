...
###' 
###' Version 2.0 - "To the point"
###' 
###' 
###' << 02_INTERMEDIATE/MIDDLE SCHOOLS >>
###' 
###' 
###' (1) Mean Effects
###' 
...
###' MIDDLE SCHOOLS: Exclude subjects other than ELA and Math for now

df_y$y_name
to_exclude <- c("_Social", "_Science", "FoLan", 
                "_CTE", "Other", "_SelfCon", "Art", "PE",  
                "Apprvd", "AP")
vars_to_include <- !grepl(paste(to_exclude, collapse = "|"), df_y$y_name)
tinker_cond <- df_y$y_name == "PCT_Other"
...
###'######################################################################
###'
###' Loop over outcome variables: i 
###'
###'

for (i in 55:length(y_names_vec)){
  ...
  ### Filter only Traditional schools => Exclude Charter schools 
  tabdf(df_temp, Traditional_SOC)
  tabdf(df_temp, Charter)
  
  df_temp <- df_temp %>%
    filter(Traditional_SOC %in% c("Intermediate/Middle/Junior High")) %>%
    filter(Charter != 1)
  ...
  }  # end of loop for generate tables on Error cells
  ...
  ###' 
  ###' Start a loop over reference years
  ###'
  ###'
  ...
  for (yr in seq_along(ref_year_vec)){
    ...
    ###' Start with a medium number of predictors (14 categories)
    ...
    ###'######################################################################
    ###'
    ###' Start a loop over tau20, tau50, mean, and tau80
    ###'
    ###'
    ...
    for (k in seq_along(element_vec)){
      ...
      ###'######################################################################
      ###'
      ###' End of a loop over tau20, tau50, mean, and tau80
      ###'
      ###'
    ...
    ###'######################################################################
    ###'
    ###' Save the resulting Step 2 estimates
    ###'
    ###'
    ...
    ###'######################################################################
    ###' 
    ###' End for loop over two reference years: 2012-13, 2013-14
    ###' outcome variables: i
    ###' 
    ###' 
