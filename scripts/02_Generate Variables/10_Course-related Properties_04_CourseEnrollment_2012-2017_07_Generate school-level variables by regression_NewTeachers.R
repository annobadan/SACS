...
###' - ONLY NewTeaching and NewInDistrict
###' 
###' => Problem occurred when filtering estimates
###' 
###'  
###' 
###' 20181231 JoonHo Lee
...
###' 1) i = years
###' 2) j = outcome
###' 3) k = predictor => only NewTeching & NewInDistrict
###'
###'
...
setwd(data_dir)
write.csv(df_error, 
          file = "Within-school teacher sorting regression_01_Error Info_NewTeachers.csv")
...
setwd(data_dir)
dualsave(df_sumstats, 
         "Within-School teacher sorting regression_02_Summary Stats_NewTeachers")
...
setwd(data_dir)
dualsave(df_est_long, 
         "Within-School teacher sorting regression_03_Estimates_Long_NewTeachers")
...
x_lookup <- setNames(c("NewT", "NewInD"), 
                     unique(df_temp$x))
...
setwd(data_dir)
dualsave(df_est_wide, 
         "Within-School teacher sorting regression_04_Estimates_Wide_NewTeachers")



###'######################################################################
###'
###' Append/Merge into the main files
###'
###'

### Summary statistics
setwd(data_dir)
load(file = "Within-School teacher sorting regression_02_Summary Stats.rda")
df_sumstats_main <- df_to_save

names(df_sumstats)
names(df_sumstats_main)

df_sumstats_bind <- bind_rows(df_sumstats_main, df_sumstats) %>%
  arrange(x, y, Subject_Category, AcademicYear)

tabdf(df_sumstats_bind, x)

dualsave(df_sumstats_bind, "Within-School teacher sorting regression_02_Summary Stats_Final")


### School-level estimates (1) Long data
load(file = "Within-School teacher sorting regression_03_Estimates_Long.rda")
df_est_long_main <- df_to_save

names(df_est_long)
names(df_est_long_main)

df_est_long_bind <- bind_rows(df_est_long_main, df_est_long) %>%
  arrange(CountyCode, DistrictCode, SchoolCode, x, y, Subject_Category, year)

tabdf(df_est_long_bind, x)

dualsave(df_est_long_bind, "Within-School teacher sorting regression_03_Estimates_Long_Final")



### School-level estimates (2) Wide data
load(file = "Within-School teacher sorting regression_04_Estimates_Wide.rda")
df_est_wide_main <- df_to_save

names(df_est_wide)
names(df_est_wide_main)

df_est_wide_merged <- df_est_wide_main %>%
  full_join_track(df_est_wide, 
                  by = c("CountyCode", "DistrictCode", "SchoolCode", "AcademicYear"), 
                  .merge = TRUE)

tabdf(df_est_wide_merged, .merge)

df_est_wide_final <- df_est_wide_merged %>%
  select(CountyCode, DistrictCode, SchoolCode, AcademicYear, 
         contains("_EL_"),
         contains("_WhiteS_"), 
         contains("_LatinoS_"), 
...
names(df_est_wide_final)

setwd(data_dir)
dualsave(df_est_wide_final, 
         "Within-School teacher sorting regression_04_Estimates_Wide_Final")
