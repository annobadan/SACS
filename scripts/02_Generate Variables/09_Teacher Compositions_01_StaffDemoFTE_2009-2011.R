...
###' Generate Variables
###' 
###' School-level Teacher Compositions 2009-2011
###' 
...
for (i in seq_along(years)) {
  ...
  tabdf(df, FTE_Teaching)
  tabdf(df, FTE_administrative)
  tabdf(df, FTE_PupilServices)
  
  df_Teacher <- df %>%
    filter(FTE_Teaching != 0)
  
  tabdf(df_Teacher, FTE_Teaching)
  ...
  ### Generate StaffType Dummy variables
  df <- df %>%
    mutate(Dummy_Teaching = if_else(FTE_Teaching != 0, 1, 0, 
                                    missing = NA_real_), 
           Dummy_administrative = if_else(FTE_administrative != 0, 1, 0, 
                                          missing = NA_real_), 
           Dummy_PupilServices = if_else(FTE_PupilServices != 0, 1, 0, 
                                         missing = NA_real_))
  ...
  df_StaffType <- school_composition_count(df_temp, 
                                           var_to_count = RecID, 
                                           factor = StaffType, 
  ...
  df_Teacher_Gender <- school_composition_count(df_Teacher, 
                                                var_to_count = RecID, 
                                                factor = GenderCode,
                                                levels_to_replace = levels_to_replace, 
  ...
  df_Teacher_Race <- school_composition_count(df_Teacher, 
                                              var_to_count = RecID, 
                                              factor = EthnicGroup, 
                                              levels_to_replace = levels_to_replace,
  ...
  df_Teacher_Education <- school_composition_count(df_Teacher, 
                                                   var_to_count = RecID, 
  ...
  df_Teacher_NewTeaching <- school_composition_count(df_Teacher, 
                                                     var_to_count = RecID, 
                                                     factor = NewTeaching, 
                                                     levels_to_replace = levels_to_replace,
  ...
  df_Teacher_NewInDistrict <- school_composition_count(df_Teacher, 
                                                       var_to_count = RecID, 
  ...
  df_Teacher_EmployStatus <- school_composition_count(df_Teacher, 
                                                      var_to_count = RecID, 
  ...
  ### Generate FTE long data
  df_temp_All_Staffs <- df %>%
    select(CountyCode, DistrictCode, SchoolCode, RecID, 
           starts_with("FTE_")) %>%
    gather(key = "key", value = "FTE", starts_with("FTE_")) %>%
    arrange(CountyCode, DistrictCode, SchoolCode, RecID, key)
  
  df_temp_Teacher_Only <- df_Teacher %>%
    select(CountyCode, DistrictCode, SchoolCode, RecID, 
           starts_with("FTE_")) %>%
    gather(key = "key", value = "FTE", starts_with("FTE_")) %>%
    arrange(CountyCode, DistrictCode, SchoolCode, RecID, key)
