...
###' Note: 
###' 
###' A record is created for every class, gender and ethnicity in a school 
###' that has students enrolled on Information Day and 
###' includes "unduplicated" count of students in a class. 
###'         
###' 
###' 20181209 JoonHo Lee
###' 
...
for (i in seq_along(years)) {
  ...
  load(file = paste0("ClassEnrollment", year_num, "_cleaned", ".rda"))
  names(df)
  ...
  df_Gender <- df %>%
    ...
    group_by(CountyCode, DistrictCode, SchoolCode, 
             ClassID, ClassCourseCodeOrMultCourseInd, GenderCode) %>%
  ...
  df_RaceEL <- df %>% 
    ...
    group_by(CountyCode, DistrictCode, SchoolCode, 
             ClassID, ClassCourseCodeOrMultCourseInd, key) %>% 
  ...
  df_Grade <- df %>%
    ...
    group_by(CountyCode, DistrictCode, SchoolCode, 
             ClassID, ClassCourseCodeOrMultCourseInd, GradeLevelCode) %>%
    summarise(N = sum(EnrollTotal, na.rm = TRUE)) %>% 
  ...
  dualsave(df_wide, paste0("ClassEnrollment", year_num, "_managed"))
