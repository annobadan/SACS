...
### Get the list of Census variable
setwd(work_dir)
var17 <- load_variables(2017, "acs5", cache = TRUE)
write.csv(var17, file = "tables/Variable list_ACS_2017.csv")


###' Get data for every Census tract in California
###' 
###' B06011_001 Estimate!!Median income in the past 12 months!!Total
###' B14006_002	Estimate!!Total!!Income in the past 12 months below the poverty level
###' 
###' B23008_003 Estimate!!Total!!Under 6 years!!Living with two parents
###' B23008_004 Estimate!!Total!!Under 6 years!!Living with two parents
###'           !!Both parents in labor force
###' B23008_005 Estimate!!Total!!Under 6 years!!Living with two parents
###'           !!Father only in labor force

census_vars <- c("B06011_001", "B14006_001", 
                 "B23008_003", "B23008_004", "B23008_005")

CA_income <- map_df("CA", function(x) {
  get_acs(geography = "tract", variables = census_vars, 
          state = x)
})
...
names(CA_income_wide) <- c("GEOID", "tract_name",
                           ...
                           "N_under6kids_est", "N_under6kids_err", 
                           "N_under6_bothwork_est", "N_under6_bothwork_err", 
                           "N_under6_onlydadwork_est", "N_under6_onlydadwork_err")


### Generate proportion variables
CA_income_wide <- CA_income_wide %>%
  mutate(PCT_under6_bothwork = 100*(N_under6_bothwork_est/N_under6kids_est), 
         PCT_under6_onlydadwork = 100*(N_under6_onlydadwork_est/N_under6kids_est))

with(CA_income_wide, plot(PCT_under6_onlydadwork, log(median_inc_est)))
with(CA_income_wide, plot(PCT_under6_bothwork, log(median_inc_est)))
...
PCT_bothwork_pal <- colorQuantile(palette = "YlGn", n = 5,  
                                  domain = CA_tracts_merged$PCT_under6_bothwork)


PCT_onlydadwork_pal <- colorQuantile(palette = "YlGn", n = 5,  
                                     domain = CA_tracts_merged$PCT_under6_onlydadwork)


summary(CA_tracts_merged$median_inc_est)
summary(CA_tracts_merged$PCT_under6_bothwork)
summary(CA_tracts_merged$PCT_under6_onlydadwork)
...
Kindergarten_Map <- df_elem %>% 
  ...
  addPolygons(data = CA_tracts_shp,
              ...
              stroke = TRUE,
              color = "darkgray", 
              fillColor = ~PCT_onlydadwork_pal(PCT_under6_onlydadwork), 
              fillOpacity = 0.5) %>%
  ...
  addLegend(title = "Census Tract %Stay-at-home mothers Quintile", 
            pal = PCT_onlydadwork_pal, 
            values = CA_tracts_merged$PCT_under6_onlydadwork, 
            position = "topleft", 
            opacity = 10)
...
saveWidget(Kindergarten_Map, 
           file = "Map02_Kindergarten Program Type 2015-17_by PCT stay-at-home mom.html")
